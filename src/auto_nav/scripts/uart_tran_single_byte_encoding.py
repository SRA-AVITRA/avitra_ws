...
rot_ticks_L = 0
rot_ticks_R = 0
def deploy_payloads_in_single_byte(direction, speed):
    global motor_L, motor_R, rot_flag, rot_ticks_L, rot_ticks_R
    if direction == "w":
        motor_L = 8 + speed
        motor_R = 8 - speed #128 + 41
    elif direction == "s":
        motor_L = 8 - speed
        motor_R = 8 + speed #128 + 41
    elif direction == "a":
        ...
        motor_L = 8
        motor_R = 8
    ser.write([ (motor_L << 4) + motor_R ])
    # ser.write([motor_R])
    print "*********", "\tpl: ", "{0:b}".format((motor_L << 4) + motor_R)

def deploy_payloads_old(direction, speed):
    global motor_L, motor_R, rot_flag, rot_ticks_L, rot_ticks_R
    if direction == "w":
        motor_L = 41 + speed
...
if __name__=="__main__":
    ...
    deploy_payloads_in_single_byte("r", 0)
    direction = "r"
    speed = 0
    motor_L = 41
    motor_R = 169
                ...
                if payload < 128:   #Left
                    ...
                    if rot_flag:
                        ...
                        if abs(rot_ticks_L) >= rotation_step_size:
                            rot_flag = 0
                            direction = "r"
                            deploy_payloads_in_single_byte(direction, speed)
                    if r_received_flag:
